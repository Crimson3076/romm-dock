...
from _vendor.atlas import register_zstd_provider

from adapters.atlas_host import grant_zstd_provider
...
def _grant_the_vendored_zstd_codec():
    """Bootstrap grants this once, before the first atlas adapter is built (``adapters/atlas_host.py``).

    This test module reads a real, zstd-compressed AppImage-embedded ES-DE
    catalogue through the real vendored atlas — never through bootstrap — so
    it has to make the same grant itself. Restored to unregistered afterward,
    the same reasoning as ``tests/adapters/test_atlas_host.py``'s own fixture:
    process-global state inside the vendored package must not leak into
...
class TestRealAtlasVendoringIntegrity:
    ...
    JSON data through ``atlas/_data.py``'s ``__package__``-anchored
    ``importlib.resources.files(...)`` internally (arrangement-caveat and
    catalogue lookups) — a vendoring regression there (see ``_vendor/README.md``'s
    "A package named in a string rather than imported" entry) would raise
    ``ModuleNotFoundError: No module named 'atlas'`` from deep inside atlas,
    never from this backend's own code, so no fake-installation test can catch
    it. This class is the one place that goes through the real thing
    end-to-end, on the same minimal fixture ``TestFactoryBind`` uses.
    """
...
class TestVendoredZstdCatalogueReading:
    ...
    project's target) or ``backports.zstd``, or one was handed over through
    ``register_zstd_provider`` — neither of the first two exists in this
    runtime without vendoring one, so ``adapters/emudeck_launcher_backend.py``'s
    ...
    rather than reading the real, sealed-inside-the-AppImage catalogue. This
    module's ``_grant_the_vendored_zstd_codec`` fixture makes the same grant
    ``adapters/atlas_host.py`` makes at bootstrap, so these tests see the same
    ...
    def test_atlas_squashfs_resolves_the_vendored_provider(self):
        ...
        assert decompressor(backports_zstd.compress(payload)) == payload
