...
def _backend(
    ...
    **kwargs,
) -> RetroDeckLauncherBackend:
    return RetroDeckLauncherBackend(
        paths=FakeRetroDeckPaths(**kwargs),
        core_info=core_info if core_info is not None else FakeCoreInfoProvider(),
        sandbox_launcher=sandbox_launcher if sandbox_launcher is not None else FakeSandboxLauncher(),
    )


def _factory(**kwargs) -> RetroDeckLauncherBackendFactory:
    return RetroDeckLauncherBackendFactory(
        paths=FakeRetroDeckPaths(**kwargs), core_info=FakeCoreInfoProvider(), sandbox_launcher=FakeSandboxLauncher()
    )
...
class TestCoreInfoDelegation:
    """RetroDECK's CoreInfoProvider methods delegate to the injected vendored catalogue,
    except resolve_sandbox_launcher, which delegates to the separate injected sandbox-launcher
    seam (ES-DE's find rules)."""
    ...
    def test_resolve_sandbox_launcher_delegates(self):
        sandbox_launcher = FakeSandboxLauncher({"cmd": "/app/launcher.sh"})
        backend = _backend(sandbox_launcher=sandbox_launcher)
        assert backend.resolve_sandbox_launcher("cmd") == "/app/launcher.sh"
        assert sandbox_launcher.calls == ["cmd"]
