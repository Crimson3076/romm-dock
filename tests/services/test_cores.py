...
class TestGetPlatformCoreInfo:
    ...
    def test_windows_rom_returns_no_emulators(self, event_loop, service, uow, core_info):
        # A native-Windows ROM (ADR-0038) has no emulator/core step at all — the
...
class TestSetGameCore:
    ...
    def test_windows_rom_refused_before_any_write(self, event_loop, service, uow, core_info):
        # A native-Windows ROM has no emulator/core step (ADR-0038) — refused
...
class TestClearGameCore:
    ...
    def test_clear_windows_rom_refused(self, event_loop, service, uow, active_core):
        ...
        # (ADR-0038) — refused before the active-core resolver (which knows
...
class TestSetSystemCore:
    ...
    def test_windows_platform_is_a_no_op(self, event_loop, service, settings, settings_persister, uow):
        ...
        # (ADR-0038) — nothing is written to settings.json and no installed ROM
