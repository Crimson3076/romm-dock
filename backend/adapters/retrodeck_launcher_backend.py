...
# Protocol services depend on it through. ``core_info`` (the vendored-atlas
# catalogue — see adapters/atlas_catalogue.py) is typed the same way: it
# structurally satisfies the catalogue half of CoreInfoProvider.
# ``sandbox_launcher`` is a second, separate source (``adapters/
# es_find_rules.py``'s ``resolve_sandbox_launcher``, ES-DE's find rules rather
# than the catalogue) held apart for the reason CoreInfoProvider's docstring
# gives: which source answers is a property of the active backend, not of the
# catalogue.
...
class RetroDeckLauncherBackend:
    ...
    def __init__(self, *, paths: Any, core_info: Any, sandbox_launcher: Any) -> None:
        self._paths = paths
    ...
    def resolve_sandbox_launcher(self, command: str) -> str | None:
        return self._sandbox_launcher(command)
...
class RetroDeckLauncherBackendFactory:
    ...
    def bind(self, installation_id: str) -> RetroDeckLauncherBackend | None:
        ...
        return RetroDeckLauncherBackend(
            paths=self._paths, core_info=self._core_info, sandbox_launcher=self._sandbox_launcher
        )
