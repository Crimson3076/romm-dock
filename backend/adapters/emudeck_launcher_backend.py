...
(``domain.emulator_commands`` — the same kernel RetroDECK's own core
selection resolves through; ES-DE ``<command>`` syntax does not change
across frontends). The classified command's ``%EMULATOR_<NAME>%``
...
# the front of a command — mirrors adapters/es_find_rules.py's own
