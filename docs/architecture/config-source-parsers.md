...
| `xemu.toml`                    | TOML                    | `adapters/xemu_config.py` + `domain/xemu_config.py`                 | ✅ adapter + pure-domain parse (correct layering) | Whether xemu's own `[sys.files]` boot ROM and flash BIOS paths point at this plugin's BIOS directory — read-only, consumed by `FirmwareService.check_xemu_alignment()` for the System page's Xbox warning (see [BIOS and Emulator Core Management](../user-guide/bios-management.md#xbox-xemu)). The plugin never writes to `xemu.toml`.                                                                                    |
