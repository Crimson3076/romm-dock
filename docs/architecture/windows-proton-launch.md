...
game binary, per-DLC executables). `ExeSelector` (`frontend/src/bigpicture/ExeSelector.tsx`) is the structural twin of
...
| File                                                      | Purpose                                                                                              |
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `backend/domain/windows_launch.py`                         | Pure kernel: `enumerate_executables`, `resolve_launch_path` — no I/O                                 |
| `backend/domain/proton.py`                                 | `ProtonInstallation` value object crossing the adapters/services boundary                            |
| `backend/domain/shortcut_data.py`                          | `WINDOWS_PLATFORM_SLUG`, `resolve_proton_invocation`, the bypass branch in `_resolve_launch_options` |
