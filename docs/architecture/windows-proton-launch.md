...
recorded in [ADR-0038](../adr/0038-plugin-owns-proton-invocation.md).
...
(`frontend/src/bigpicture/ExeSelector.tsx`) is the structural twin of `DiscSelector`, mounted immediately to its right
in the play-section row on the game detail page. It renders a compact icon-only trigger for an installed
native-Windows ROM whose install enumerates at least one launchable target (neutral grey when following the default
pick, accent-tinted when a specific one is pinned) and nothing at all otherwise — unknown, not-installed, non-Windows,
and no-target ROMs all collapse to the backend's `{"has_executables": false}` answer. The frontend renders both kinds
identically (a bare filename); it never reads `kind` — that branch lives entirely in
`WindowsLaunchResolver.resolve_launch_options` on the backend.
...
(ADR-0038), so there is nothing for `CoreService` to resolve through. Each entry point guards `rom.platform_slug ==
...
| File                                                      | Purpose                                                                                            |
| --------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `backend/domain/windows_launch.py`                         | Pure kernel: `enumerate_executables`, `resolve_launch_target`, `resolve_launch_path` — no I/O       |
| `backend/domain/proton.py`                                 | `ProtonInstallation` value object crossing the adapters/services boundary                           |
| `backend/domain/shortcut_data.py`                          | `WINDOWS_PLATFORM_SLUG`, `resolve_proton_invocation`, `resolve_native_invocation`                   |
| `backend/adapters/proton_locator.py`                       | `ProtonLocatorAdapter` — Steam-root scan, tie-break rule, `compat_data_path`                        |
| `backend/services/protocols/proton.py`                     | `ProtonLocator` Protocol                                                                            |
| `backend/services/protocols/cross_service.py`              | `WindowsResolver` Protocol                                                                          |
| `backend/services/windows_launch_resolver.py`              | `WindowsLaunchResolver` — the single read-path seam                                                 |
| `backend/services/windows_game.py`                         | `WindowsGameService` — the exe-picker's two callables                                               |
| `backend/services/library/shortcut_launch_resolver.py`     | `do_scan_windows_launch_options` / `do_read_windows_launch_options`, the bake sites' entry points   |
| `backend/services/cores.py`                                | `CoreService` — refuses `platform_slug == "win"` outright (no emulator/core concept applies)        |
| `backend/db/migrations/024_add_selected_exe.sql`           | Adds `roms.selected_exe`                                                                            |
| `frontend/src/bigpicture/ExeSelector.tsx`                  | The picker UI, structural twin of `DiscSelector`                                                    |
