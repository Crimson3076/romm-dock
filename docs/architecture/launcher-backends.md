...
[ADR-0039](../adr/0039-launcher-backend-seam-and-switch-as-rebake.md) for the full decision record, including why it
...
## EmuDeck — sourced from the plugin's already-vendored emu-atlas
...
(`_vendor.atlas.detect(home)`), read off `backend/_vendor/atlas/` — the same vendored tree
`adapters/atlas_catalogue.py`, `adapters/atlas_firmware.py`, and `adapters/atlas_saves.py` already pin for RetroDECK's
own emulator catalogue, firmware, and save-location answers. This backend is a fourth consumer of that one tree, not a
second vendored copy at a different version. See `backend/_vendor/README.md` for the vendor provenance (pinned tag,
manifest, and licence).
...
(`domain/emulator_commands.py` — `classify_command`, `select_default_option`, the same pure kernel RetroDECK's own
core selection resolves through) is reused **unchanged** against atlas's catalogue text, rather than reimplemented for
a second frontend. A per-game/per-platform pin (`emulator.label`) is matched by label
...
`backend/_vendor/backports_zstd/` supplies it, handed to atlas via `atlas.register_zstd_provider()` from
`adapters/atlas_host.py` (see `_vendor/README.md`'s entry for why it's vendored and what it cost to get right).
Without a working zstd decompressor, atlas silently degrades to a catalogue built from the installed libretro cores'
own names, with no command text at all — nothing bakes, and every affected shortcut's `launch_options` end up empty
(ADR-0039's EmuDeck errata).
...
- [ADR-0039](../adr/0039-launcher-backend-seam-and-switch-as-rebake.md) — the decision record.
